...
# Start Kubernetes
docker run -d --name=etcd --net=host gcr.io/google_containers/etcd:2.0.9 \
    /usr/local/bin/etcd --addr=127.0.0.1:4001 --bind-addr=0.0.0.0:4001 --data-dir=/var/etcd/data
docker run -d --name=master --net=host -v /var/run/docker.sock:/var/run/docker.sock gcr.io/google_containers/hyperkube:v0.21.2 \
...
docker run -d --name=proxy --net=host --privileged gcr.io/google_containers/hyperkube:v0.21.2 \
